...
export default function StvExplainer({ path }: Props = {}) {
  ...
  const inputs = [
    t('stvMd.inputCandidates'),
    t('stvMd.inputSeats'),
    t('stvMd.inputVotes'),
  ];

  const transferRules = [
    t('stvMd.transferDefeated'),
    t('stvMd.transferElected'),
  ];

  const noWasteCases = [
    t('stvMd.noWasteCaseMajority'),
    t('stvMd.noWasteCaseMinority'),
  ];

  const references = [
    { url: 'https://gendignoux.com/blog/2023/03/27/single-transferable-vote.html', label: t('stvMd.refGendignoux') },
    { url: 'https://en.wikipedia.org/wiki/Single_transferable_vote#Meek\'s_method', label: t('stvMd.refMeek') },
    ...
    { url: 'https://www.youtube.com/watch?v=P38Y4VG1Ibo', label: t('stvMd.refElectoralCommission') },
  ];
        ...
        <CardContent>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 2 }}>
            {inputs.map((input, index) => (
              <Paper key={index} elevation={1} sx={{ p: 2, borderLeft: 3, borderColor: 'primary.main' }}>
        ...
        <CardContent>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, mt: 2 }}>
            {references.map((ref, index) => (
              <Paper key={index} elevation={1} sx={{ p: 2, borderLeft: 3, borderColor: 'secondary.main' }}>
                <Link
                  href={ref.url}
                  target="_blank"
